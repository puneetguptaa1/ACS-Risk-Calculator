#!/usr/bin/env python3
"""
Write Charbel's JSON risk values into the Charbel sheet of the Excel file.

Reads blank_tongue_nsqip_2024_charbel_risks.json (keyed by CASEID) and writes the
15 scraped risk values into the corresponding row of the Charbel sheet,
matching on the CASEID in column B.

Usage:
    python json_to_excel_charbel.py
    python json_to_excel_charbel.py --json blank_tongue_nsqip_2024_charbel_risks.json --excel blank_tongue_nsqip_2024.xlsx
"""
...
# JSON key -> Excel column letter (same layout as Anahita's sheet)
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(description="Write Charbel's JSON risk values into the Charbel sheet.")
    ap.add_argument("--json",  type=Path, default=root / "blank_tongue_nsqip_2024_charbel_risks.json")
    ap.add_argument("--excel", type=Path, default=root / "blank_tongue_nsqip_2024.xlsx")
    ap.add_argument("--sheet", default="Charbel")
    ...
    log = logging.getLogger("json_to_excel_charbel")
    ...
    wb.save(excel_path)
    log.info(
        "Done. %d rows written, %d rows skipped (no JSON entry).",
        written, missing,
    )
